  ...
  cats.forEach(cat => {
    ...
    items.forEach(item => {
      ...
      html += '<div class="card" style="border-color:rgba(212,135,28,.3);padding:14px">' +
        ...
        '<div><div style="font-size:15px;font-weight:700;color:#FFF8E7">' + item.title + '</div>' +
...
def api_shop_items():
    ...
    try:
        conn = get_db()
        cur = conn.cursor()
        cur.execute("SELECT id, category, title, description, price_caps FROM shop_items WHERE is_active = TRUE ORDER BY category, id")
